...
<div>
	...
	<div class="progress-box cloa22">
		<?php
		$showEntriesFrontEnd = showEntriesFrontEnd($postsid); //(!empty($showEntriesFrontEnd)) ? count($showEntriesFrontEnd) : count($ListUserFirstSubs);
		?>
		<p><?php echo $valueStocks > $showEntriesFrontEnd ? $showEntriesFrontEnd : $valueStocks;?> / <?php echo $valueStocks;?></p>
		...
		<div class="bitslab-giveaway-detail-widget">
			<?php if ($countdown) { ?>
				<div class="bitslab-coundown">
					<div class="bitslab-countdown-timer" data-datetime="<?= $countdown ?>" data-giveaway-id="<?= $postsid ?>">
						<div id="days"></div>
						<div id="hours"></div>
						<div id="minutes"></div>
						<div id="seconds"></div>
